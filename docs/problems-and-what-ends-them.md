...
**migkit: Not yet, and this one bites migkit itself.** The repair path
builds `insert into ... ("id", "v") values (...) on conflict ("id") do
update set ...`, with no `OVERRIDING` clause. That exact statement was run
against a `GENERATED ALWAYS` target and produced the error above; adding
the clause made it succeed. So `migkit apply` cannot repair a row on a
target whose key is a generated-always identity column.

It fails **loudly**, with that error, rather than writing something wrong -
which is the right way round, and is why this is a gap rather than an
emergency. But the tool should either emit the clause or say up front that
the target will not accept the keys it is carrying.
